<template>
    ...
    <div class="matched_talent_info">
        ...
        <MatchedTalentList_m v-if="check" v-for="(results,index) in result" :key ="index" v-bind:result="results" :bridge = "bridge"/>
        <Message_m v-if="message"  :name={name} :myId = {myId}  :youId = {youId} />
        <Add_Memo_m v-if="memo"  :datas = {memo_list} />
